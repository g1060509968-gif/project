...
3. [新增] 输出最终光谱数据到 CSV 文件 (Wavelength, Normalized Intensity)
"""
...
def test_spectrum_pulse():
    ...
    # ==========================================
    output_csv_name = "output_final_spectrum.csv"
    ...
    try:
        ...
        intensity_shifted = np.abs(np.fft.fftshift(spectrum_complex))**2
        ...
        intensity_final = intensity_shifted[valid_mask]
        
        # lambda = c / f
        wl_nm = (solver.c / f_abs) * 1e9
        
        # 4. 归一化强度
        if np.max(intensity_final) > 0:
            intensity_norm = intensity_final / np.max(intensity_final)
        else:
            intensity_norm = intensity_final
            
        # 5. 排序（因为频率和波长成反比，FFT后的波长是从大到小的，需要反转以方便Origin）
