...
def analyze_beam_radius(R, lambda_nm, current_d, config, wm_range=None):
    ...
    # 设置刻度朝内，四周都有边框 (典型的 Scientific Plot 风格)
    ...
    if wm_range:
        ...
        if C_target is not None:
            ...
            ax.vlines(x=C_target, ymin=w0_at_target, ymax=target, 
                      ...
                      label=f'Target $w_m$={target}mm', zorder=10)
        ...
        if C_min is not None:
            ...
            ax.vlines(x=C_min, ymin=w0_at_min, ymax=lower, 
                      ...
                      label=f'Min Limit $w_m$={lower:.3f}mm', zorder=9)
            
        # --- B. 添加不同颜色的色块 (Fill Between) ---
        # 填充区域：C 在 [C_min, C_target] 之间，且在 w0 和 wm 曲线之间
        if C_min is not None and C_target is not None:
            # 创建布尔掩码
            fill_mask = (C_axis >= C_min) & (C_axis <= C_target)
            
            # 填充色块：使用淡绿色 + 斜线纹理，表示“可接受的设计窗口”
            ax.fill_between(C_axis, w0_curve, wm_curve, where=fill_mask,
                            color='#2ECC71', alpha=0.25, hatch='///', edgecolor='none',
                            label='Design Window', zorder=1)
    ...
    # 5. 图表修饰 (Labels, Title, Grid, Legend)
...
def main():
    ...
    # 容差范围: [target_wm + tol_min, target_wm + tol_max]
    # 例如: -1.5 到 +0.3 意味着允许光斑在 (0.5-1.5) 到 (0.5+0.3) 之间
