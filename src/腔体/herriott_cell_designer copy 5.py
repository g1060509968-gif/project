...
def find_configs_by_spot_size(f, lambda_nm, target_wm, tol_min=-0.01, tol_max=0.01, min_nu=10, max_nu=50, max_mu_limit=None):
    ...
    # 打印表格
    header = f"{'Idx':<4} | {'ν (光斑数)':<10} | {'模式(ν,μ)':<10} | {'d (双凹)':<10} | {'d/R':<8} | {'w0 (mm)':<8} | {'wm (mm)':<8} | {'Type':<6}"
    print("-" * len(header))
    print(header)
    print("-" * len(header))
    ...
    for i, res in enumerate(results):
        ...
        print(f"{i:<4} | {res['v']:<10} | ({res['v']}, {res['mu']}):<10 | {res['d']:.2f}{'':<4} | {res['C']:.3f}    | {res['w0']:.4f}   | {res['wm']:.4f}   | {type_str}")
...
def plot_stability_diagram_for_selection(f, d, results, max_mu_limit=None):
    ...
    # 绘制所有搜索到的解，横坐标转为 d/R
    # d/f 转换 d/R: d/R = d/(2f)
    dR_vals = [r['d']/R for r in results]
    v_vals = [r['v'] for r in results]
    
    plt.scatter(dR_vals, v_vals, c='blue', alpha=0.6, label='满足光斑条件的解')
    
    # 标记当前选中的解
    current_dR = d/R
    plt.axvline(x=current_dR, color='red', linestyle='--', alpha=0.8, label=f'当前选择 d/R={current_dR:.3f}')
    
    title_suffix = f"(μ <= {max_mu_limit})" if max_mu_limit else "(无 μ 限制)"
    plt.title(f"符合目标光斑大小的稳定性分布 {title_suffix}")
    plt.xlabel("稳定性参数 C = d/R (腔长/曲率半径)")
    plt.ylabel("单面镜光斑数 (ν)")
    
    # 因为 R = 2f, 稳定区是 0 < d < 4f => 0 < d < 2R => 0 < d/R < 2
    plt.xlim(0, 2)
...
def analyze_beam_radius(R, lambda_nm, current_d, config, wm_range=None, all_valid_results=None):
    """
    计算并绘制光斑半径随 C 值变化
    【修改】：横坐标为 d/R，并将所有有效解的数据点叠加到曲线上
    """
    ...
    # 3. 叠加所有满足条件的解 (将散点图数据合并进来)
    if all_valid_results:
        # 提取所有解的坐标
        valid_C = [res['d'] / R for res in all_valid_results]
        valid_wm = [res['wm'] for res in all_valid_results]
        valid_w0 = [res['w0'] for res in all_valid_results]
        
        # 绘制到 wm 曲线上
        ax.scatter(valid_C, valid_wm, color='#FF0000', s=30, alpha=0.5, marker='o', 
                   edgecolors='none', zorder=20, label='Valid Solutions ($w_m$)')
        # 绘制到 w0 曲线上 (可选，展示对应腰斑)
        # ax.scatter(valid_C, valid_w0, color='#3498DB', s=15, alpha=0.4, marker='x', 
        #            zorder=5, label='Valid Solutions ($w_0$)')

    # 4. 处理目标线、最小值线、最大值线及填充色块
    if wm_range:
        ...
        if C_min is not None and C_target is not None:
            ...
            ax.fill_between(C_axis, w0_curve, wm_curve, where=fill_mask_lower,
                            ...
                            label='Min-Target Range', zorder=1)
        ...
        if C_target is not None and C_max is not None:
            ...
            ax.fill_between(C_axis, w0_curve, wm_curve, where=fill_mask_upper,
                            ...
                            label='Target-Max Range', zorder=1)

    # 5. 标记当前设计点 (Selected Point)
    ax.scatter([current_C], [config['w0']], color='#C0392B', s=100, marker='*', edgecolors='white', linewidth=1, zorder=11)
    ax.scatter([current_C], [config['wm']], color='#C0392B', s=100, marker='*', edgecolors='white', linewidth=1, zorder=11, label='Selected Config')

    # 6. 图表修饰
    ax.set_xlabel(r'Stability Parameter $C = d/R$ (Cavity Length / Curvature)', fontsize=12, weight='bold')
    ax.set_ylabel(r'Beam Radius $w$ (mm)', fontsize=12, weight='bold')
    ax.set_title(f'Herriott Cell Beam Radius vs. Stability (C=d/R)\n(R={R}mm, $\lambda$={lambda_nm}nm)', fontsize=13, pad=15)
...
def main():
    ...
    print(f"  稳定性参数 C     : {selected_config['C']:.4f} (d/R)")
    print("#" * 50)

    # 3. 绘制稳定性分布 (修改版：横坐标 d/R)
