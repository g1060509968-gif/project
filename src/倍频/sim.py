...
# [子图 2] 最终级：5HG 时域波形与脉冲形变 (采用双 Y 轴)
ax1 = plt.subplot(2, 2, 2)
# 注意这里画的是 A_4w_aligned
...
# [子图 3] 全光路光谱演化 (频域)
plt.subplot(2, 2, 3)
plt.plot(nu_THz + c/lam_1w*1e-12, get_spectrum(A1_depleted_1), 'r', label='1w 剩余光谱')
plt.plot(nu_THz + c/lam_2w*1e-12, get_spectrum(A_2w), 'g', label='2w 光谱')
plt.plot(nu_THz + c/lam_4w*1e-12, get_spectrum(A_4w), 'b', label='4w 光谱')
plt.plot(nu_THz + c/lam_5w*1e-12, get_spectrum(A_5w), 'm', label='5w 光谱')
plt.xlim(200, 1600)
plt.ylim(60, 200)
plt.xlabel('绝对频率 (THz)'), plt.ylabel('谱密度 (dB)')
plt.title('各阶谐波绝对光谱分布')
plt.legend()
plt.grid(True, alpha=0.3)

# [子图 4] 脉冲形变与色散分析 (归一化对比)
plt.subplot(2, 2, 4)
