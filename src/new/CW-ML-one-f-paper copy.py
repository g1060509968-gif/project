...
class LaserConfig:
    def __init__(self):
        ...
        self.P_cw = 1.0          # 连续光计算功率 (W)（本程序中仍用 power=0 的线性稳态作对比）
        ...
        # --- 优化参数 ---
        self.max_round_trips = 50  # 最大允许圈数
...
def _is_stable_round_trip(M_rt):
    ...
    A = M_rt[0, 0]
    D = M_rt[1, 1]
    return (abs((A + D) / 2.0) <= 1.0)


def calculate_stability_abcd(cfg, z, x, theta_deg):
    """
    计算全腔往返矩阵 (Full Round Trip Matrix) 并判稳。
    往返矩阵 M_rt = M_backward @ M_forward
    """
    ...
    M_cry_t = np.array([[1.0, L_eff_t], [0.0, 1.0]])
    M_cry_s = np.array([[1.0, L_eff_s], [0.0, 1.0]])

    M_da = np.array([[1.0, d_a], [0.0, 1.0]])
    M_db = np.array([[1.0, d_b], [0.0, 1.0]])
    M_L1 = np.array([[1.0, cfg.L1], [0.0, 1.0]])
    M_L2 = np.array([[1.0, cfg.L2], [0.0, 1.0]])

    def get_full_round_trip_matrix(f_val, M_cry_spec):
        M_f = np.array([[1.0, 0.0], [-1.0 / f_val, 1.0]])

        # Forward: End1 -> End2 (通过 L1 -> M1 -> Crystal -> M2 -> L2)
        # 注意矩阵乘法顺序是逆序应用的 (Last Element @ ... @ First Element)
        M_fwd = M_L2 @ M_f @ M_db @ M_cry_spec @ M_da @ M_f @ M_L1

        # Backward: End2 -> End1 (Reverse path)
        M_bwd = M_L1 @ M_f @ M_da @ M_cry_spec @ M_db @ M_f @ M_L2

        return M_bwd @ M_fwd

    M_rt_t = get_full_round_trip_matrix(ft, M_cry_t)
    M_rt_s = get_full_round_trip_matrix(fs, M_cry_s)

    stable_t = _is_stable_round_trip(M_rt_t)
    stable_s = _is_stable_round_trip(M_rt_s)
...
class BPM_Engine_GPU:
    ...
    def get_beam_width_gpu(self, E_gpu):
        """
        返回 (wx, wy)，用二阶矩（D4σ 类似定义）：
            w = 2*sqrt(Var)
        修正点：
        - 对 Ix/Iy 分别用各自的一维归一化（不能用二维 total）
        - CuPy 标量判断使用 .item()
        """
        I_gpu = cp.abs(E_gpu) ** 2
        total_2d = cp.sum(I_gpu)

        if total_2d.item() == 0.0:
            return 0.0, 0.0

        Ix = cp.sum(I_gpu, axis=0)  # x 方向边缘积分
        Iy = cp.sum(I_gpu, axis=1)  # y 方向边缘积分
        ...
        wx = calc_width_1d(Ix, self.x_gpu)
        wy = calc_width_1d(Iy, self.y_gpu)
        return wx, wy
    ...
    def run_simulation(self, z, x, theta, power, trace_mode=False):
        ...
        # 预计算常用传播算子
        step_half = (self.cfg.L_crys / 2) / self.cfg.bpm_steps
        ...
        def single_round_trip(E_in_gpu, record=False):
            ...
            for _ in range(self.cfg.bpm_steps):
                E_in_gpu = self.propagate(E_in_gpu, step_half, self.cfg.n0)
                if power > 1.0:
                    E_in_gpu = self.apply_kerr(E_in_gpu, step_half)
                if record:
                    curr_d += step_half
                    rec(E_in_gpu, curr_d)

            # 2) Arm 2 (Long Arm): gap d_b -> mirror -> L2 -> return -> mirror -> gap d_b
            E_in_gpu, curr_d = prop_air(E_in_gpu, d_b, curr_d)
            E_in_gpu = self.apply_lens(E_in_gpu, ft, fs)
            E_in_gpu, curr_d = prop_air(E_in_gpu, self.cfg.L2, curr_d)
            E_in_gpu, curr_d = prop_air(E_in_gpu, self.cfg.L2, curr_d)
            E_in_gpu = self.apply_lens(E_in_gpu, ft, fs)
            E_in_gpu, curr_d = prop_air(E_in_gpu, d_b, curr_d)
            ...
            for _ in range(self.cfg.bpm_steps):
                ...
                if power > 1.0:
                    E_in_gpu = self.apply_kerr(E_in_gpu, step_full)
                if record:
                    curr_d += step_full
                    rec(E_in_gpu, curr_d)

            # 4) Arm 1 (Short Arm): gap d_a -> mirror -> L1 -> return -> mirror -> gap d_a
            E_in_gpu, curr_d = prop_air(E_in_gpu, d_a, curr_d)
            E_in_gpu = self.apply_lens(E_in_gpu, ft, fs)
            E_in_gpu, curr_d = prop_air(E_in_gpu, self.cfg.L1, curr_d)
            E_in_gpu, curr_d = prop_air(E_in_gpu, self.cfg.L1, curr_d)
            E_in_gpu = self.apply_lens(E_in_gpu, ft, fs)
            E_in_gpu, curr_d = prop_air(E_in_gpu, d_a, curr_d)

            # 5) Crystal Front (half)
            for _ in range(self.cfg.bpm_steps):
                E_in_gpu = self.propagate(E_in_gpu, step_half, self.cfg.n0)
                if power > 1.0:
                    E_in_gpu = self.apply_kerr(E_in_gpu, step_half)
...
if __name__ == "__main__":
    ...
    # X轴: 20点
    x_scan = np.linspace(-2e-3, 2e-3, 10)

    k_map = np.zeros((len(x_scan), len(z_scan)))
    print("\n>>> [Step 2] 计算稳定性图谱 (Grid, Early Stopping Enabled)...")
    ...
    for i, x in enumerate(tqdm(x_scan, desc="Scanning")):
        for j, z in enumerate(z_scan):
            ...
            if not (st_t and st_s):
                ...
                continue
            try:
                w_cw = bpm.run_simulation(z, x, best_theta, power=0, trace_mode=False)
                w_ml = bpm.run_simulation(z, x, best_theta, power=cfg.P_peak, trace_mode=False)
                if w_cw > 0:
    ...
    plt.title("KLM Stability Map (ABCD round-trip fixed)")
    plt.xlabel("Z (mm)")
    plt.ylabel("X (mm)")

    # --- 边缘切除逻辑 ---
    k_search = k_map.copy()
    edge_cut = 10  # 配合 100 点精度，切除 1mm（每侧）
